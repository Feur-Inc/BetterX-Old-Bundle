...
function validateConfig(data) {
    ...
    const theme_state = {
        order: Array.isArray(data.theme_state?.order) ? data.theme_state.order.filter((i) => typeof i === "string") : [],
        active: Array.isArray(data.theme_state?.active) ? data.theme_state.active.filter((i) => typeof i === "string") : []
    };
...
app.get("/api/config", authMiddleware, (c) => {
    ...
    const config = db.query("SELECT * FROM configs WHERE user_id = ?").get(user.id);
